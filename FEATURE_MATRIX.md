...
| Global rules | Disables natural health regeneration; enables keep inventory; removes phantoms; reduces explosion-drop loss; preserves ender pearls on death; controls time progression. | `data/matcha_flavoured_plus/function/main/setup/gamerules.mcfunction` |
| Extended days / sleep | Slows the normal day cycle and replaces normal sleeping with a managed sleep/time-advance system. | `data/matcha_flavoured_plus/function/main/environmental`, `data/matcha_flavoured_plus/function/main/mechanic/sleep*` |
| Hunger redesign | Hunger is no longer a normal survival limiter; high hunger triggers a hunger effect and low hunger gets saturation support. Food is intended as healing and buff delivery. | `data/matcha_flavoured_plus/function/main/mechanic/manage_hunger.mcfunction` |
| Food intrinsics | Food ingredients carry gameplay effects; cooking, preservation, and meals form progression chains rather than simple saturation upgrades. Every `minecraft:sweet_berries` stack grants Regeneration III for one second when consumed; the heart lore on pack-generated berries is presentation only. | 122 recipe definitions in `data/matcha_flavoured_plus/recipe/food`; `data/matcha_flavoured_plus/advancement/main/mechanics/sweet_berries_eaten.json` |
...
| Death model | Keep inventory; deaths deduct one permanent heart rather than deleting carried items. | `data/matcha_flavoured_plus/function/main/mechanic/hpdown.mcfunction` |
| Crystal Hearts | Permanent maximum-health upgrades, capped at 30 hearts. | `data/matcha_flavoured_plus/function/main/mechanic/process_heart_container.mcfunction` |
...
| Early utility | Basic ceramics and reduced inventory friction; Easy Shulker Boxes and Tom's Storage provide the modded storage layer in the full pack. | Crafting and tutorial definitions plus installed storage mods. |
...
| Metal/alloy paths | Bronze, Steel, Electrum, Shakudo/Palatinate, and Adamant gear paths; alloy-specific tools, armour, and progression gates. | 70 smithing recipes in `data/matcha_flavoured_plus/recipe/smithing_table` |
| Steel progression | Carbon-Rich Iron intermediary, then Steel Alloy via blasting; steel emphasizes durability and defensive properties. | `data/matcha_flavoured_plus/recipe/crafting/carbon_rich_iron.json`, `data/matcha_flavoured_plus/recipe/blasting/steel_alloy.json` |
...
| Endless repairs | Tools are intended to remain repairable indefinitely with appropriate base materials; XP is controlled to prevent vanilla-style progression from dominating. | `data/matcha_flavoured_plus` and anvil/XP functions. |
| XP redesign | Continuously removes normal XP while temporarily granting controlled XP for repair-related flows. | `data/matcha_flavoured_plus/function/main/mechanic/remove_xp.mcfunction`, `set_xp.mcfunction` |
| Crafted enchanting | Enchantments are acquired through recipes/blessings rather than enchantment-table randomness and XP grinding. | 23 blessing recipes in `data/matcha_flavoured_plus/recipe/blessings` |
| Custom enchantments | Anemos, Bloodrage, four Cleanses, Conduit Power, Divinity, Fire Proof, Frost Protection, Haste, Reach, Regeneration, Riposte, Sanguine, Slaughter, Traversal, Warding Armour, four Warding tiers, and Zephyr. | 23 definitions in `data/matcha_flavoured_plus/enchantment/main` |
| Estus system | Raw Estus is processed into a healing/resistance effect and Estus Ash; Stabilised Estus participates in higher progression. | `data/matcha_flavoured_plus/function/main/mechanic/check_if_estus_should_be_processed.mcfunction` |
...
| Warding Stones | Placeable base-defense system: wards nearby undead, harms them, produces effects, and regenerates allied villagers/friends. | `data/matcha_flavoured_plus/function/main/mechanic/warding_stone*.mcfunction` |
| Trial Chamber constraint | Warding Stones are forcibly removed and trigger an explosion/effects in Trial Chambers. | `data/matcha_flavoured_plus/advancement/main/mechanics/enter_trial_chamber.json` |
...
| Mob balance | Fast zombies, modified husks, half-health skeletons, weaker creepers, fast low-health cave spiders, zero equipment-drop chance, and altered undead drops. | `data/matcha_flavoured_plus/function/main/spawn_mechanic`, entity loot tables. |
| Dragon aftermath | Killing the first Dragon activates a safer surface state: many hostile surface spawns are removed while underground danger remains. | `data/matcha_flavoured_plus/function/main/first_dragon_killed_reward.mcfunction`, `safe_surface.mcfunction` |
...
| Stonecutting overhaul | Extensive stonecutter catalog, including generous conversions and building-material access. | `data/matcha_flavoured_plus/recipe/stonecutting` |
...
| Freezing water | Water/freezing environmental hazard with Slowness, Darkness, and freeze damage. | `data/matcha_flavoured_plus/function/main/environmental/freezing_water.mcfunction` |
| Music | Custom music-disc content, jukebox-song definitions, and wandering-trader music access. | `data/matcha_flavoured_plus`, `data/matcha_flavoured_plus/jukebox_song/main`, trade data. |
...
|---|---|
| Food, healing, hunger | `data/matcha_flavoured_plus/function/main/mechanic`, `data/matcha_flavoured_plus/recipe/food`, food loot tables |
| Death and maximum health | `hpdown.mcfunction`, `process_heart_container.mcfunction`, Crystal Heart recipes/loot |
| Mob ecology / Dragon world-state | `data/matcha_flavoured_plus/function/main/spawn_mechanic`, `first_dragon_killed_reward.mcfunction`, entity loot |
| Warding | `warding_stone*.mcfunction`, Trial Chamber advancement |
| Gear and alloys | `data/matcha_flavoured_plus/recipe/smithing_table`, `data/matcha_flavoured_plus/recipe/crafting`, `data/matcha_flavoured_plus/recipe/blasting` |
| Enchanting / Divine gating | `data/matcha_flavoured_plus/recipe/blessings`, `data/matcha_flavoured_plus/enchantment/main`, loot tables |
