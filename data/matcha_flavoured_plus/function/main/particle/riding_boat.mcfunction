...
# execute unless entity @p[nbt={RootVehicle:{Entity:{id:"minecraft:pale_oak_boat"}}}] run advancement revoke @p only matcha_flavoured_plus:main/particle/riding_boat
execute as @a[scores={boating=1..}] run scoreboard players set @a boating 0
# execute if entity @p[nbt={RootVehicle:{Entity:{id:"minecraft:pale_oak_boat"}}}] run execute as @p[scores={boating=1..}] on vehicle run schedule function matcha_flavoured_plus:main/particle/riding_boat 3t