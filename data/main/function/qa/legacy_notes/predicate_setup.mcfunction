...
execute as @e[type=minecraft:armor_stand,tag=matcha_qa_predicate,limit=1] at @s if predicate main:sky_spawn run say MATCHA_QA_PREDICATE_PASS
execute as @e[type=minecraft:armor_stand,tag=matcha_qa_predicate,limit=1] at @s unless predicate main:sky_spawn run say MATCHA_QA_PREDICATE_FAIL
kill @e[type=minecraft:armor_stand,tag=matcha_qa_predicate]
